...
public class MidiFile
{
  ...
  public MidiFile(LinkedList<Staves> sl)
  {
	staveList = sl;
  ...
  /** Store a note-on event followed by a note-off event a note length
      later. There is no delta value Ñ the note is assumed to
		...
		for (int j=0; j<staveList.size(); j+=1)
		{
			...
			for (int k=0; k<tempSymbol.size(); k+=1)
			{
				...
				for (int i=0; i<notes.size(); i+=1)
				{
					...
					this.noteOnOffNow(tempNote.getDuration(),tempNote.getNote()+64, 127);
				}
  ...
  /** Test method Ñ creates a file test1.mid when the class
      is executed 
