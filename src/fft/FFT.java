...
public class FFT
{
	...
	private int size;
	private Bitmap fftImage;
	private Complex c1;
	private Coordinates minCord;
	private Coordinates maxCord;

	public FFT(Bitmap buffImage, int size)
	{
		this.size = size;
		c1 = new Complex(size);
		...
		this.fftImage = Bitmap.createBitmap(size/2, size/2, Bitmap.Config.ARGB_8888);
	...
	{
		return (float) Math.atan((double) (maxCord.x - minCord.x) / (double) (maxCord.y - minCord.y));
	}

	public Bitmap getFFTImage()
	{
		return fftImage;
	}
		...
		for (int i = start1; i < end1; i += 1)
		{
			...
			for (int j = start2; j < end2; j += 1)
			{
				...
				if (n > BRIGHT_THRESHOLD)
				{
					...
					fftImage.setPixel(x + hj, y + hi, (n << 16) | (n << 8) | (n));
			}
		}
	}
